...
export function TaskCard({ task, onEdit, onDelete }: TaskCardProps) {
  ...
  return (
    <motion.div variants={itemVariants} layout exit="exit" style={{ transformStyle: 'preserve-3d' }}>
      <TiltCard className="group h-full p-4">
        <div className="flex h-full flex-col gap-3">
          <div className="flex items-start justify-between gap-2">
            <h3 className="line-clamp-2 font-semibold text-white">{task.title}</h3>
            <div className="flex shrink-0 gap-1 opacity-60 transition-opacity duration-200 group-hover:opacity-100">
              <button
                type="button"
                onClick={() => onEdit(task)}
